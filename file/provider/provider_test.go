...
func TestGetTestOpenSession(t *testing.T) {
	...
	assert.Equal(t, providerDisplayName, provider.VolumeProvider("VPC-FILE"))
	vpcs.Close()

	providerName := vpcs.ProviderName()
	assert.Equal(t, providerName, provider.VolumeProvider("VPC-FILE"))

	volumeType := vpcs.Type()
	assert.Equal(t, volumeType, provider.VolumeType("vpc-file"))
