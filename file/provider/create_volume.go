...
func (vpcs *VPCSession) CreateVolume(volumeRequest provider.Volume) (volumeResponse *provider.Volume, err error) {
	vpcs.Logger.Debug("Entry of CreateVolume for file method...")
	defer vpcs.Logger.Debug("Exit from CreateVolume for file method...")
	defer metrics.UpdateDurationFromStart(vpcs.Logger, "CreateVolume", time.Now())

	vpcs.Logger.Info("Basic validation for CreateVolume file request... ", zap.Reflect("RequestedVolumeDetails", volumeRequest))
	resourceGroup, err := validateVolumeRequest(volumeRequest)
	if err != nil {
		return nil, err
	}
	vpcs.Logger.Info("Successfully validated inputs for CreateVolume file request... ")
	...
	err = retry(vpcs.Logger, func() error {
		volume, err = vpcs.Apiclient.VolumeFileService().CreateVolume(volumeTemplate, vpcs.Logger)
		return err
	})
...
func validateVolumeRequest(volumeRequest provider.Volume) (models.ResourceGroup, error) {
	...
	//TBD validate Profiles and targets
	...
	if volumeRequest.Capacity == nil {
		return resourceGroup, userError.GetUserError("VolumeFileCapacityInvalid", nil, nil)
	} else if *volumeRequest.Capacity < minSize || *volumeRequest.Capacity > maxSize {
		return resourceGroup, userError.GetUserError("VolumeFileCapacityInvalid", nil, *volumeRequest.Capacity)
	}
