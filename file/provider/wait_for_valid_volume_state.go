...
func WaitForValidVolumeState(vpcs *VPCSession, volumeID string) (err error) {
	...
	vpcs.Logger.Info("Getting volume details from VPC file provider...", zap.Reflect("VolumeID", volumeID))

	var volume *models.Volume
	err = retry(vpcs.Logger, func() error {
		volume, err = vpcs.Apiclient.VolumeFileService().GetVolume(volumeID, vpcs.Logger)
		if err != nil {
			return err
		}
